...
import { Filters, AuthorFilter } from 'src/core/database/filters';
import { Sorting, DateSort } from 'src/core/database/sorting';
...
function selectAllDrafts(f: AuthorFilter, s: DateSort, o: Offset, l: Limit) {
  const filterQuery = filters(f);
  const [sortingQuery, sortField, sortType] = sorting(s);
  const limitQuery = limitation(o, l);
...
export { selectAll, selectAllDrafts };
